/*
    ...
    ***ルール***
    一般的なオセロのルールに準拠する。
    奇数ターンは黒のターン,偶数ターンは白のターン。
    緑で示されている場所に石を置くことができる。
    石を置きたい場所をマウスでクリックすると石を置くことができる。
    石を置くことができない場所にマウスをクリックすると石を置くことができない。
    石を置くことができる場所がない場合はスキップすることができる。
    **********

    ***工夫した点***
    1.石を置くことができる場所を緑で表示するようにした。
    2.石を置くことができる場所がない場合はパスすることができるようにした。
    3.石を置くことができる場所がない場合は石の数を数えて勝敗を決めるようにした。
    4.どんなスクリーンでもウィンドウがスクリーンの真ん中に表示されるようにした。
    5.GitHub Copilotを使用して制作時間を大幅に短縮した。
    (主にコメントの記述とOthello_Algorithm関数とCAN_PUT関数の作成に使用した。)
    **********

    ***分かっている不具合***
    1.石を反転中に画面を連打すると石が予期せぬ場所に置かれる。
    2,緑の表示が正しくない場合が稀にある。
    **********
...
#define WHITE 0  // 白のターン
#define BLACK 1  // 黒のターン
